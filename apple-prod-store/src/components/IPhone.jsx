            ...
            <div className="flex justify-between w-full">
                {iphoneModels.map((iphone, index) => (
                    <div key={index} className="flex flex-col items-center justify-between gap-y-2">
                        ...
                        <p className="2xl:text-lg xl:text-base lg:text-sm text-sm text-gray-700 mt-2">{iphone.description}</p>
                        <span className="2xl:text-base xl:text-sm lg:text-xs text-blue-400 mt-2">{iphone.price}</span>
                        <button type='button' className="cursor-pointer text-sm mt-4 px-4 py-2 bg-blue-400 text-white rounded-full">Buy</button>
                    </div>
                ))}
            </div>
        </div>
